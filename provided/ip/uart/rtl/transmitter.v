...
module transmitter#(parameter baud_rate_count= 108)
(
...
initial begin state <=0; nextstate <=0; counter <= 0; bitcounter<=0; rightshiftreg <= 0;  end
	   ...
	   begin // reset is asserted (reset = 1)
        ...
        bitcounter <=0; //counter for bit transmission is reset to 0
         ...
         counter <= counter + 1; //counter for baud rate generator start counting 
            if (counter >= baud_rate_count)//10415) //if count to 10416 (from 0 to 10415)
               begin 
